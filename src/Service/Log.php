...
class Log
{
     ...
     *     array(
     * @varchar "type" - action type ,object "old" - old object ,object "new" - new object
     *     )
     *
    ...
    public static function add(&$p)
    {
        ...
        $dispatcher->dispatch('beforelogadd', new BeforeLogAddEvent($p));
        ...
        $dispatcher->dispatch('logadd', new LogAddEvent($p));
    ...
    protected static function getLogData(&$p)
    {
        ...
        $allOldData = empty($p['old']) ? [] : $p['old']->getData();

        $allNewData = empty($p['new']) ? [] : $p['new']->getData();
    ...
    protected static function getActivityData(&$d)
    {
        ...
        $rez['fu'] = array_diff($rez['fu'], [$userId]);

        $rez['fu'] = Util\toNumericArray($rez['fu']);

        if (!empty($d['sys_data']['wu'])) {
            $rez['wu'] = array_diff($d['sys_data']['wu'], [$userId]);
